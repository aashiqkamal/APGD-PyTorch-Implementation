#Attack wrappers class for APGD bs=32
import torch 
...
def get_grad(model, x, y, loss_fn): #Return per-sample input gradients 
    ...
    for i in range(B): # single sample 
        xi = x[i:i+1].detach().clone().requires_grad_(True)  # xi - ith image, adds a batch dimension, fresh tensor,   dloss/dxi. 
...
def APGDNativePytorch(device, dataLoader, model, eps_max, num_steps,
    ...
    for x_clean, y in dataLoader:
        ...
        for i in range(bs):
            f_x0[i] = loss_fn(model(x_clean[i:i+1]), y[i:i+1]).sum()
            f_x1[i] = loss_fn(model(x_next[i:i+1]),  y[i:i+1]).sum()

        better = (f_x1 > f_x0)        # [bs] bool [true, false, true, ...], which indices improved 
        x_max  = x_clean.clone()
        x_max[better] = x_next[better] # pick the better image 
        f_max  = f_x0.clone()
        f_max[better] = f_x1[better] # pick the corresponding higher loss 
        ...
        eta = torch.full((bs,1,1,1), eta_scalar, device=device, dtype=x_clean.dtype) # shape, one value per sample filling with 2 * eps_max, 
        ...
        for k in range(1, num_steps):
            ...
            if k  == W[checkpoint_ptr]:
                ...
                same_eta_flat = (eta == prev_eta).all(dim=(1, 2, 3))  # [bs] True/False per sample,  true: eta unchanged for this sample 
                
                cond2 = same_eta_flat & (f_max == prev_f_max)          #  [bs]
